...
use crate::cli::args::DiskInspectArgs;
use crate::cli::commands::exit_for_disk_error;
use crate::config::load::load_config;
use crate::disk::{
    device_path_for_uuid, ensure_disk_not_mounted, mount_options_for_restore, select_disk,
};
...
pub fn run_inspect(config_path: &Path, _args: DiskInspectArgs, disk_id: Option<&str>) -> Result<()> {
    ...
    let mountpoint = create_temp_dir(Path::new(&cfg.user_mount_base), "tv-inspect")?;
    ...
    let status = Command::new(shell)
        ...
        .status()
        .map_err(|e| TimevaultError::message(format!("launch shell: {}", e)))?;
    if !status.success() {
