...
- `remote.minimumUptimeSeconds`: Fallback threshold used after WOL only when the current-boot systemd sleep journal cannot be read. Default: `600` seconds. Normally Timevault distinguishes a resumed host from a cold boot using a recent `systemd-sleep` resume record, so a cold-booted host stays classified as powered off even if it took more than 10 minutes to become reachable.
...
- `return` (default): put the host back in the power state Timevault found it in — left running if it answered without Wake-on-LAN, suspended (`systemctl suspend`) if WOL resumed it from suspend, powered off (`systemctl poweroff`) if WOL cold-booted it. For WOL, Timevault checks for a current-boot systemd sleep/resume record within the readiness window; an older record means the host was already running. It uses `minimumUptimeSeconds` only if the journal query is unavailable.
